...
function parseMessageSigners(messageBytes: Uint8Array): {
    ...
    const { length: numStaticAccounts, bytesConsumed } = decodeShortVecLength(
        messageBytes.subarray(offset)
    );
...
function extractSignatureAtIndex(
    ...
    if (signedTx instanceof Uint8Array) {
        ...
        if (numSignatures !== expectedNumSigners) {
            console.warn(
                `⚠️ Gill: Signature count mismatch - expected ${expectedNumSigners}, found ${numSignatures}`,
            );
        }
        
        if (signerIndex >= numSignatures) {
            throw new Error(
                `Signer index ${signerIndex} out of range (transaction has ${numSignatures} signatures)`,
            );
        }
    ...
    if (isWeb3jsTransaction(signedTx)) {
        ...
        if (signerIndex >= signatures.length) {
            throw new Error(`Signer index ${signerIndex} out of range (transaction has ${signatures.length} signatures)`);
        }
            ...
            return signedTransactions.map((signedTx, index) => {
                ...
                try {
                    ...
                    if (signedTxBytes.length !== wireFormat.length) {
                        ...
                        const originalWithLifetime = originalTransaction as any;
                        const result = {
                            ...walletTransaction,
