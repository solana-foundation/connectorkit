...
export function useTransactions(options: UseTransactionsOptions = {}): UseTransactionsReturn {
    ...
    const parseTransaction = useCallback((
        ...
        try {
            ...
            const walletIndex = accountKeys.findIndex(
                (key: string) => key.toLowerCase() === walletAddress.toLowerCase()
            );
            ...
            const programIds = new Set(
                (message.instructions || []).map((ix: any) => 
                    accountKeys[ix.programIdIndex] || ix.programId
                ).filter(Boolean)
            );
            ...
            if (hasJupiter || hasOrca || hasRaydium) {
                ...
                if (accountKeys.length >= 2) {
                    counterparty = accountKeys.find((key: string, idx: number) => 
                        idx !== walletIndex && 
                ...
                const ourPreTokens = preTokenBalances.filter((b: any) => 
                    accountKeys[b.accountIndex]?.toLowerCase() === walletAddress.toLowerCase() ||
                    b.owner?.toLowerCase() === walletAddress.toLowerCase()
                );
                const ourPostTokens = postTokenBalances.filter((b: any) => 
                    accountKeys[b.accountIndex]?.toLowerCase() === walletAddress.toLowerCase() ||
                    b.owner?.toLowerCase() === walletAddress.toLowerCase()
                );
                ...
                if (!tokenMint && ourPostTokens.length > ourPreTokens.length) {
                    type = 'received';
                    direction = 'in';
                    const newToken = ourPostTokens.find((b: any) => 
                        !ourPreTokens.some((p: any) => p.mint === b.mint)
                    );
                    if (newToken) {
        ...
        try {
            ...
            if (fetchDetails && signaturesResult.length > 0) {
                ...
                const txPromises = signaturesResult.map(s => 
                    rpc.getTransaction(toSignature(String(s.signature)), {
                        encoding: 'jsonParsed',
    ...
    }, [connected, address, rpcClient, cluster, limit, beforeSignature, fetchDetails, parseTransaction]);
